{{ config(
    ...
    post_hook = ["
                    ...
                    {% endif %}
                    ","delete from Staging.placements_recruiters_stg where _airbyte_extracted_at != (select max(_airbyte_extracted_at) from Staging.placements_recruiters_stg)"],
    tags = [ "top-level" ]
) }}
...
new_data as (
    ...
    -- placements_recruiters from {{ source('airbyte_internal', 'jobadder_destv2_raw__stream_placements') }}
    where 1 = 1
